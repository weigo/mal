...
SpecialSexpressionHandlerSpec *new_sepecial_s_exression_handler(
    void (*continue_handler)(MalValue **, MalEnvironment **),
    MalValue *(*return_handler)(MalCell *, MalEnvironment *),
    bool do_return)
{
...
MalValue *is_macro_call(MalValue *value, MalEnvironment *environment)
{
    ...
    MalValue *v = lookup_in_environment(environment, get_current_package(), head);
...
MalValue *eval_ast(MalValue *value, MalEnvironment *environment)
{
    ...
    case MAL_SYMBOL:
    {
        MalValue *symbol = lookup_in_environment(environment, get_current_package(), value);
...
void let_star(MalValue **value, MalEnvironment **environment)
{
    ...
    if (!head->cdr || !head->cdr->value || !is_sequence(head->cdr->value))
    {
...
void if_(MalValue **value, MalEnvironment **environment)
{
    ...
    else if (!is_nil(condition) && !is_false(condition))
    {
        *value = head->cdr->cdr->value;
    }
...
MalValue *EVAL(MalValue *value, MalEnvironment *external_environment)
{
    MalValue *current = value;
    MalEnvironment *environment = external_environment;
...
char *get_history_filename()
{
    ...
    char *history_file = mal_malloc(strlen(home_folder) + strlen(HISTORY_FILENAME) + 1 + 1);
...
int main(int argc, char **argv)
{
    ...
    // FIXME: add those symbols to the environment of mal-user
    MalValue *package = get_current_package();
    set_in_environment(package->package->environment, make_symbol("*ARGV*"), args);
    set_in_environment(package->package->environment, make_symbol("*host-language*"), make_string("c.3", false));
    ...
    rep("(println (str \"Mal [\" *host-language* \"]\"))", package->package->environment, true);
    ...
    while (1)
    {
        ...
        {
            _add_history(input);
            rep(input, package->package->environment, true);
        }
        else
        {
            break;
